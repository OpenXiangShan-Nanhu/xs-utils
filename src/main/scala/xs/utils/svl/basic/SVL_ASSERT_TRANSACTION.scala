...
object SVL_ASSERT_TRANSACTION {
  def apply(
    ...
    val assertTransaction = Module(new assert_transaction(
      severity_level = severity_level,
      width = width,
      property_type = property_type,
      msg = msg,
    ...
    assertTransaction.io.clk := clock
    assertTransaction.io.reset_n := ~reset.asBool
    assertTransaction.io.test_expr := test_expr
    assertTransaction.io.start_state := start_state
    assertTransaction.io.next_state := next_state
    assertTransaction.suggestName(name)
  }
}
