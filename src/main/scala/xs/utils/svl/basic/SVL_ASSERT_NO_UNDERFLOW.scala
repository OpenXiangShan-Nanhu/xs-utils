...
object SVL_ASSERT_NO_UNDERFLOW {
  def apply(
    name: String,
    clock: Clock,
    reset: Reset,
    ...
    val assertNoUnderflow = Module(new assert_no_underflow(
      severity_level = severity_level,
      width = width,
      min = min,
      max = max,
    ...
    assertNoUnderflow.io.clk := clock
    assertNoUnderflow.io.reset_n := ~reset.asBool
    assertNoUnderflow.io.test_expr := test_expr
    assertNoUnderflow.suggestName(name)
  }
}
